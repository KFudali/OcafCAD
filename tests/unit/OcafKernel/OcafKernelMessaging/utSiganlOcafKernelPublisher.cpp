...
struct MockSubscriber {
    ...
    MOCK_METHOD(void, onComponentAddedToAssembly, (const ComponentAddedToAssemblyEvent&), ());
    MOCK_METHOD(void, onComponentRemovedFromAssembly, (const ComponentRemovedFromAssemblyEvent&), ());
};
...
TEST_F(SignalOcafKernelPublisherTest, PrototypeEventsArePublished){
    ...
    EXPECT_CALL(mock, onPrototypeAdded(testing::Field(&PrototypeAddedEvent::label, label))).Times(1);
    EXPECT_CALL(mock, onPrototypeRemoved(testing::Field(&PrototypeRemovedEvent::label, label))).Times(1);
...
TEST_F(SignalOcafKernelPublisherTest, PartEventsArePublished){
    ...
    mMessageBus->subscribe<PartAddedEvent>([&](const PartAddedEvent& e){ mock.onPartAdded(e); });
    mMessageBus->subscribe<PartRemovedEvent>([&](const PartRemovedEvent& e){ mock.onPartRemoved(e); });
    mMessageBus->subscribe<PartLocationChangedEvent>([&](const PartLocationChangedEvent& e){ mock.onPartLocationChanged(e); });
    mMessageBus->subscribe<PartAttributeChanged>([&](const PartAttributeChanged& e){ mock.onPartAttributeChanged(e); });

    PartLabel partLabel;
    PartAttributeType attr = PartAttributeType::Color;

    EXPECT_CALL(mock, onPartAdded(testing::Field(&PartAddedEvent::label, partLabel))).Times(1);
    EXPECT_CALL(mock, onPartRemoved(testing::Field(&PartRemovedEvent::label, partLabel))).Times(1);
    EXPECT_CALL(mock, onPartLocationChanged(testing::Field(&PartLocationChangedEvent::label, partLabel))).Times(1);
    EXPECT_CALL(mock, onPartAttributeChanged(
        testing::AllOf(
...
TEST_F(SignalOcafKernelPublisherTest, ComponentAssemblyEventsArePublished){
    ...
    mMessageBus->subscribe<ComponentAddedToAssemblyEvent>(
        [&](const ComponentAddedToAssemblyEvent& e){ mock.onComponentAddedToAssembly(e); }
    );
    mMessageBus->subscribe<ComponentRemovedFromAssemblyEvent>(
        [&](const ComponentRemovedFromAssemblyEvent& e){ mock.onComponentRemovedFromAssembly(e); }
    );

    PartLabel compLabel;

    EXPECT_CALL(mock, onComponentAddedToAssembly(testing::Field(&ComponentAddedToAssemblyEvent::label, compLabel))).Times(1);
    EXPECT_CALL(mock, onComponentRemovedFromAssembly(testing::Field(&ComponentRemovedFromAssemblyEvent::label, compLabel))).Times(1);
