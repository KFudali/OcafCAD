...
TEST(ProgressRangeTest, SubrangeCreationAndWeight) {
    ...
    auto& child1 = root.newChild(Fraction(0.6f), "Child1");
    ...
    auto& child2 = root.newChild(Fraction(0.4f), "Child2");
...
TEST(ProgressRangeTest, ParentProgressPropagation) {
    ProgressRange root("Root");
    auto& child = root.newChild(Fraction(0.5f), "Child");
...
TEST(ProgressRangeTest, ParentProgressRetractOnSetToAndReset) {
    ProgressRange root("Root");
    auto& child = root.newChild(Fraction(1.0f), "Child");
...
TEST(ProgressRangeTest, NewChildThrowsIfChildAlreadyExists) {
    ...
    auto& child1 = root.newChild(Fraction(0.5f), "Child1");
    EXPECT_EQ(child1.label(), "Child1");

    EXPECT_THROW({
        root.newChild(Fraction(0.5f), "Child2");
    }, std::logic_error);
}
...
TEST(ProgressRangeTest, ClearChildFinalizesAndRemovesChild) {
    ...
    auto& child = root.newChild(Fraction(0.0f), "Child");
